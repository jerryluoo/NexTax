...
then runs the NexTax pipeline to produce example redacted outputs.
...
def generate_output_examples() -> None:
    ...
    from nextax.tax.forms import FilingStatus, TaxInput, W2Income, F1099Int, F1099Nec
    from nextax.tax.optimizer import analyze

    print("\n📤 Generating example outputs...")

    # 1. Redacted W-2 text
    with tempfile.TemporaryDirectory() as tmp:
        pipeline = NexTaxPipeline(vault_password="demo", vault_dir=Path(tmp) / "v")
        result = pipeline.process(SAMPLES_DIR / "w2_digital.pdf", skip_ai=True)

    with open(OUTPUT_DIR / "redacted_w2.txt", "w") as f:
        f.write("# NexTax Redacted Output — W-2\n")
    ...
    with open(OUTPUT_DIR / "redacted_1099.txt", "w") as f:
        f.write("# NexTax Redacted Output — 1099-INT\n\n")
    ...
    with open(OUTPUT_DIR / "optimization_report.txt", "w") as f:
        f.write("# NexTax — Tax Optimization Report\n")
