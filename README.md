...
[![PyPI](https://img.shields.io/pypi/v/nextax.svg)](https://pypi.org/project/nextax/)
[![MIT License](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)
[![Python 3.10+](https://img.shields.io/badge/python-3.10+-blue.svg)](https://python.org)
[![Tests](https://img.shields.io/badge/tests-159%20passing-brightgreen.svg)](#tests)

📦 **Install:** `pip install nextax` — [View on PyPI](https://pypi.org/project/nextax/0.1.0/)
...
NexTax is a **local-first privacy layer** that sits between your tax documents and AI. It:
...
NexTax uses **multiple layers of defense** to prevent PII leakage:
...
### What NexTax Does NOT Protect Against
...
- **NexTax is not tax advice software.** The tax calculator is for estimation. Consult a CPA for filing.

---

## Tax Data Sensitivity Levels (DSL)

NexTax classifies every piece of tax data by sensitivity level, adapted from enterprise data security frameworks. This classification **drives all redaction decisions** — it's not guesswork, it's a formal policy.

| DSL | Level | Description | NexTax Action | Risk if Exposed |
|-----|-------|-------------|------------------|-----------------|
...
> **If NexTax detects DSL 5 data in your documents, it will warn you and refuse to process it.** These credentials should be entered directly into IRS systems, never shared with any third party including AI.

### How DSL Drives Redaction

```python
from nextax.tax.data_sensitivity import get_fields_safe_for_ai, get_fields_to_redact
...
NexTax includes a **complete federal tax calculator** for tax year 2024 that follows the IRS Form 1040 flow:
...
After computing your tax, NexTax analyzes your return and suggests:
...
Before NexTax sends anything to AI, review what will be sent:
```bash
nextax inspect w2.pdf    # Dry run — shows redacted text, nothing sent
```

### 2. Process Your Documents
```bash
nextax process w2.pdf --task extract     # Extract structured data
nextax process w2.pdf --task advise -q "Am I eligible for EITC?"
nextax process w2.pdf 1099-int.pdf --task file   # Filing preparation
```

### 3. Understand AI Responses
Claude's response uses the same tokens. NexTax **automatically restores** your real PII:
...
from nextax.tax.forms import FilingStatus, TaxInput, W2Income
...
- ⚠️ **Consult a qualified tax professional** — NexTax provides estimates, not tax advice
...
```bash
git clone https://github.com/jerryluoo/NexTax.git
cd NexTax
...
pytest --cov=nextax    # With coverage report
```
...
NexTax is a **privacy tool and tax estimation tool**, not a certified tax preparation product.
