"""Shared fixtures and helpers for NexTax integration tests."""
...
def has_tesseract() -> bool:
    ...
    from nextax.extraction.ocr_extractor import is_tesseract_available
...
def pipeline(tmp_path):
    """Create a NexTaxPipeline (in-memory only, no disk vault)."""
    from nextax.pipeline import NexTaxPipeline
    return NexTaxPipeline(
...
def pipeline_with_vault(tmp_path):
    """Create a NexTaxPipeline with persistent disk vault for testing."""
    from nextax.pipeline import NexTaxPipeline
    return NexTaxPipeline(
