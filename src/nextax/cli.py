"""NexTax CLI — command-line interface for privacy-preserving tax processing.

Usage:
    nextax process w2.pdf --task extract
    nextax process w2.pdf --task advise --query "What deductions can I take?"
    nextax inspect w2.pdf
    nextax vault list
    nextax vault clean
"""
...
from nextax.ai.claude_client import TaskType
...
@click.group()
@click.version_option(package_name="nextax")
def main():
    """🔐 NexTax — Privacy-preserving tax assistant.
...
def process(pdf_files, task, query, persist_vault, ocr, output, include_secrets, verbose):
    ...
    Examples:
        ...
        nextax process w2.pdf

        nextax process w2.pdf --task advise -q "Am I eligible for EITC?"

        nextax process w2.pdf 1099-int.pdf --task file --persist-vault
    """
    setup_logging(verbose)

    from nextax.pipeline import NexTaxPipeline

    task_type = TaskType(task)

    click.echo("🔐 NexTax — Privacy-Preserving Tax Assistant")
    click.echo("=" * 50)
    click.echo()
...
def inspect(pdf_files, ocr, verbose):
    ...
    Examples:
        ...
        nextax inspect w2.pdf --ocr
    """
    setup_logging(verbose)

    from nextax.pipeline import NexTaxPipeline

    click.echo("🔍 NexTax — Document Inspector (Dry Run)")
    click.echo("=" * 50)
    ...
    pipeline = NexTaxPipeline(persist_vault=False)
...
def vault_list(vault_dir):
    ...
    from nextax.vault.secure_vault import SecureVault
...
def vault_clean(vault_dir, force):
    ...
    from nextax.vault.secure_vault import DEFAULT_VAULT_DIR
